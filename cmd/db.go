...
type DBViewCmd struct {
	...
	Delete DBViewDeleteCmd `cmd:"" help:"Delete a database view"`
}
...
type DBViewDeleteCmd struct {
	Database string `arg:"" help:"Database URL, name, or ID"`
	View     string `arg:"" help:"View URL, view:// ID, name, or ID"`
	JSON     bool   `help:"Output as JSON" short:"j"`
}

func (c *DBViewDeleteCmd) Run(ctx *Context) error {
	ctx.JSON = c.JSON
	return runDBViewDelete(ctx, c)
}

func runDBViewDelete(ctx *Context, cmd *DBViewDeleteCmd) error {
	mcpClient, webClient, dbID, err := resolveDBAndPrivateClient(ctx, cmd.Database)
	if err != nil {
		return err
	}
	...
	view, err := webClient.DeleteDatabaseView(bgCtx, privateapi.ViewDeleteRequest{
		DatabaseID: dbID,
		ViewID:     viewID,
	})
	if err != nil {
	...
	output.PrintSuccess("View deleted")
	if view.Name != "" {
		output.PrintInfo(view.Name)
	}
	return nil
}
