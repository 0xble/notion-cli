...
func TestRunPagePropertyGetByPropertyID(t *testing.T) {
	...
	err := runPagePropertyGet(&Context{JSON: true, APIBaseURL: srv.URL + "/v1", APIToken: "test-token"}, pageID, "", propertyID)
	if err != nil {
		t.Fatalf("runPagePropertyGet: %v", err)
	}
	if gotPath != "/v1/pages/"+pageID+"/properties/"+propertyID+"?page_size=100" {
...
func TestRunPagePropertyGetByName(t *testing.T) {
	...
	err := runPagePropertyGet(&Context{JSON: true, APIBaseURL: srv.URL + "/v1", APIToken: "test-token"}, pageID, "People", "")
	if err != nil {
		t.Fatalf("runPagePropertyGet: %v", err)
	}
	...
	if paths[1] != "/v1/pages/"+pageID+"/properties/prop_people?page_size=100" {
